...
inputfile::inputfile()
{
	...
	if( !(ifile.is_open()) )
	{
		cerr<<"\nError: In opening the file input.txt\n\tIf input.txt is not exist then create manually and write sudoku((9x9) in it. FIll empty space as zero";
		throw;
	}
...
inputfile::~inputfile()
{
	if( ifile.rdstate() )
	{
		cerr<<"The ifstream object is not in good state\n"<<endl;
		throw;
	}
	else
		ifile.close();
